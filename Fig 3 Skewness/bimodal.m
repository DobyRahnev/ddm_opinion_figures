%---------------------------------------------
% bimodal
% Use DDM to produce bimodal RT distributions by employing
...
bound_upper = a - (1 - exp(-([1:50]/40).^10)) * a/2;
bound_lower = (1 - exp(-([1:50]/40).^10)) * a/2;
...
figure
subplot(2,1,1); hist(rt(choice==1),1:.5:100); ylim([0, max(hist(rt(choice==1),1:.5:100))+1000]); xlim([0, max(rt) + 10]);
title('RT for correct')
subplot(2,1,2); hist(rt(choice==0),1:.5:100); ylim([0, max(hist(rt(choice==1),1:.5:100))+1000]); xlim([0, max(rt) + 10]);
title('RT for error')
xlabel('Time (a.u.)')
ylabel('RT count')